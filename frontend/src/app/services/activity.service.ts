...
export class ActivityService {
  private apiUrl = 'http://localhost:8082/activities';
  ...
  enroll(userId: number, activityId: number): Observable<any> {
    ...
    return this.http.post<any>('http://localhost:8082/enrollments', enrollmentData, { headers });
  }

  getUserEnrollments(userId: number): Observable<any[]> {
    const token = this.authService.getToken();
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
    return this.http.get<any[]>(`http://localhost:8082/enrollments/user/${userId}`, { headers });
  }

  unenroll(userId: number, activityId: number): Observable<any> {
    const token = this.authService.getToken();
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
    return this.http.delete<any>(`http://localhost:8082/enrollments/${userId}/${activityId}`, { headers });
  }

  getEnrollmentsByActivity(activityId: number): Observable<any[]> {
    const token = this.authService.getToken();
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
    return this.http.get<any[]>(`http://localhost:8082/enrollments/activity/${activityId}`, { headers });
  }
}
